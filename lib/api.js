...
var api = {
    ...
    createCachedAPI: function(config) {
        var root = __dirname.replace(vPath.normalize('/node_modules/extdirect/lib'),''),
            ...
            responseNamespace = 'Ext.ns("' + config.namespace +'");' + config.namespace + '.' + config.apiName + '=',
            response  = {
        ...
        for(; i < len; i++){
            ...
            if(config.classPrefix && actionName.substr(0, config.classPrefix.length) === config.classPrefix) {
                apiFile = require(vPath.join(dxPath, files[i]));
                response.actions[actionName] = [];

                for(methodName in apiFile) {
                    reflection = api.reflection(apiFile[methodName]);
                    if(reflection) {
                        actionDescription = {
                            name: methodName,
